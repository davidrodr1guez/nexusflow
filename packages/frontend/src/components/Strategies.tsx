...
const STRATEGIES: Strategy[] = [
  {
    ...
    description: 'Scans yield opportunities across Ethereum, Arbitrum, and Base. Auto-compounds rewards and rebalances into highest APY pools.',
    apy: '12.4%',
    risk: 'Medium',
    status: 'active',
    pnl: '+$1,247.80',
...
export function Strategies() {
  const { agentState, connected, startPolling, stopPolling } = useAgentStore();
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {STRATEGIES.map((strategy) => {
          ...
          const displayStatus = connected ? (isAgentActive ? 'active' : strategy.status) : strategy.status;
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      <div
                        className={`w-1.5 h-1.5 rounded-full ${
                          displayStatus === 'active' ? 'bg-nexus-accent animate-pulse' : 'bg-nexus-muted'
                        }`}
                      />
